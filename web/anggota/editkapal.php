...
if(!isset($_SESSION['id_anggota'])) header("location:../../index.php");
	...
	$bahan= $_POST['bahan'];
		...
		// update user data
		$result = mysqli_query($connect, "UPDATE data_kapal SET namakapal='$namakapal', namaperusahaan='$namaperusahaan' , kapasitas='$kapasitas', tahun='$tahun', bahan='$bahan' WHERE id_kapal=$id_kapal");
...
$result = mysqli_query($connect, "SELECT * FROM data_kapal WHERE id_kapal=$id_kapal");
 
while($user_data = mysqli_fetch_array($result))
{	
	$id_kapal = $user_data['id_kapal'];
	$namakapal = $user_data['namakapal'];
	$bendera = $user_data['bendera'];
	$namaperusahaan = $user_data['namaperusahaan'];
	  ...
	  <li class="nav-item active">
        ...
        <div id="collapsePages" class="collapse show" aria-labelledby="headingPages" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
			...
			<a class="collapse-item active" href="kapal.php">Kapal</a>			
          </div>
        </div>
      </li>
        ...
        <div class="container-fluid">
		  ...
		  <!-- ISI -->
			<div class="card shadow mb-4">
				<div class="card-body">
					 <form id="update_kapal" name="update_kapal" action="editkapal.php" method="POST">
						<h1>UBAH DATA KAPAL</h1>

							<!-- One "tab" for each step in the form: -->
								<div class="tab">
									<label>ID KAPAL</label>  
									<input readonly type="text" name="id_kapal" id="id_kapal" class="form-control" value="<?php echo $id_kapal;?>" />  
									<br />  
									<label>BENDERA KAPAL</label>  
									<input type="text" name="bendera" id="bendera" class="form-control" value="<?php echo $bendera;?>"/>  
									<br />
									<label>NAMA KAPAL</label>  
									<input type="text" name="namakapal" id="namakapal" class="form-control" value="<?php echo $namakapal;?>"/>  
									<br />
									...
									<br />
									<label>KAPASITAS ANGKUT KAPAL</label>  
									<input type="text" name="kapasitas" id="kapasitas" class="form-control" value="<?php echo $kapasitas;?>"/>  
									<br />
									<label>TAHUN PEMBUATAN KAPAL</label>  
									<input type="text" name="tahun" id="tahun" class="form-control" value="<?php echo $tahun;?>"/>  
									<br />
									<label>BAHAN PEMBUATAN KAPAL</label>  
									<select name="bahan" id="bahan" class="form-control" />
										<option>- Pilih Bahan Pembuatan Kapal -</option>
										<option <?php echo ($bahan == "Fibreglass" ? "selected" : "")?>>FIBREGLASS</option>
										<option <?php echo ($bahan == "Aluminium" ? "selected" : "")?>>ALUMINUIM</option>
										<option <?php echo ($bahan == "Baja/Besi" ? "selected" : "")?>>BAJA/BESI</option>
									  </select>
									<br />
									
								</div>
								<button name="update" type="submit" class="btn btn-success"><i class="fa fa-save"></i> Simpan</button>	
								<a href="kapal.php"><button type="button" class="btn btn-danger tombol"><i class="fa fa-times"></i> Cancel</button></a>								
					  </form> 
                </div>
            </div>
