...
if(isset($_GET['dosen'])){
	...
	$tahun = $_GET['tahun'];
	$pra = $_GET['pra'];
	$hsl = mysqli_query($connect, "SELECT * FROM hasil WHERE id_dosen='$dosen' AND semester='$semester' AND tahun='$tahun' AND pra='$pra' AND id_prodi='$prodi'");
	$kues = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM kuesioner"));
	$nama = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM data_dosen WHERE id='$dosen'"));
	$tanya1 = 0;
	$tanya2 = 0;
	$tanya3 = 0;
	$tanya4 = 0;
...
$pdf->Cell(3, 0.8, ':      '.$jml_mhs.' Mahasiswa', 0, 0, 'L');
$pdf->ln(0.8);
