...
if(isset($_GET['prodi'])){
	...
	$tahun = $_GET['tahun'];
	$pra = $_GET['pra'];
	$hsl = mysqli_query($connect, "SELECT * FROM hasil WHERE id_prodi='$prodi' AND kelas='$kelas' AND semester='$semester' AND tahun='$tahun' AND pra='$pra'");
	...
	while($t_mhs = mysqli_fetch_array($mhs)){
		$nim_mhs = $t_mhs['nim'];
		$cek = mysqli_fetch_array(mysqli_query($connect, "SELECT kelas FROM data_mhs WHERE nim='$nim_mhs' AND prodi='$prodi'"));
		if($cek[0]==$kelas){
			$tot_mhs = $tot_mhs + 1;
		}
	}
...
$pdf->ln(1);
$pdf->Cell(4, 0.8, 'Kelas', 0, 0, 'L');
$pdf->Cell(3, 0.8, ':      '.$kelas, 0, 0, 'L');
$pdf->ln(0.8);
$pdf->Cell(4, 0.8, 'Program Studi', 0, 0, 'L');
