...
if(isset($_GET['makul'])){
	...
	$tahun = $_GET['tahun'];
	$pra = $_GET['pra'];
	$hsl = mysqli_query($connect, "SELECT * FROM hasil WHERE id_makul='$makul' AND semester='$semester' AND tahun='$tahun' AND pra='$pra' AND id_prodi='$prodi'");
	$kues = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM kuesioner"));
	$nama = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM data_makul WHERE id='$makul'"));
	$tanya1 = 0;
	$tanya2 = 0;
	$tanya3 = 0;
	$tanya4 = 0;
	...
	$mhs = mysqli_fetch_array(mysqli_query($connect, "SELECT count(*) FROM hasil_mhs WHERE tahun=$tahun AND pra='$pra' AND semester='$semester' AND makul='$makul'"));
	$tot_mhs = $mhs[0];
}
...
$pdf->ln(1);
$pdf->Cell(4, 0.8, 'Mata Kuliah', 0, 0, 'L');
